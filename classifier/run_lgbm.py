...
from data_processor.example2dataset_vec import load_and_cache_examples
from parm import *
...
def evaluate(args, x_dev, y_dev, clf):

    # 模型预测
    y_pred = clf.predict(x_dev, num_iteration=clf.best_iteration_)
...
def main(args):
    ...
    if args.do_train:
        train_dataset = load_and_cache_examples(args, clf_data_processor, data_type='train')
    ...
    if args.do_eval:
        print('evaluate')
        eval_dataset = load_and_cache_examples(args, clf_data_processor, data_type='dev')
        ...
        evaluate(args, x_dev, y_dev, clf)



    if args.do_test:
        print('test')
        test_dataset = load_and_cache_examples(args,clf_data_processor, data_type='test')
